...
export function HermesLogo() {
  return (
    <svg
      ...
      fillRule="evenodd"
      height="1em"
      style={{ flex: "none", lineHeight: 1, color: "var(--accent)" }}
      viewBox="0 0 24 24"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
...
function AgentLogo({ agent }: { agent: AgentKind }) {
  if (agent === "codex") return <CodexLogo />;
  if (agent === "hermes") return <HermesLogo />;
  return <ClaudeLogo />;
}
  ...
  return (
    <div className="cli-wrap">
      ...
      <div className="cli-menu" id="cliMenu" role="listbox">
        ...
        <button
          className={`cli-opt${activeAgent === "hermes" ? " sel" : ""}`}
          data-id="hermes"
          type="button"
          role="option"
          aria-selected={activeAgent === "hermes"}
          onClick={() => onSelectAgent("hermes")}
        >
          <span className="logo">
            <HermesLogo />
          </span>
          <div className="cli-meta">
            <b>Hermes</b>
            <div className="cli-path">{hermesCommandPath ?? "~/.local/bin/hermes"}</div>
          </div>
          {activeAgent === "hermes" ? <span className="check">✓</span> : null}
        </button>
      </div>
    </div>
  );
